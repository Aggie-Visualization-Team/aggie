...
FacebookContentService.prototype._doFetch = function(options, callback) {
  ...
  graph.get(source, {}, function (err, res) {
    if (res.id) {
      sourceId = res.id;
      console.log("ID -----------:" + res.id);
      callback(self._handleResults(self._doRequest('/v2.2/' + sourceId + '/feed', params, data)));
    } else {
...
FacebookContentService.prototype._doRequest = function(url, params, data) {
  var self = this;
  ...
  graph.get(url , params, function(err, res) {
      if (err) {
        ...
        if (res.paging && res.paging.next) {
          self._doRequest(res.paging.next, params, data);
        }
        return data;
      }
  });
